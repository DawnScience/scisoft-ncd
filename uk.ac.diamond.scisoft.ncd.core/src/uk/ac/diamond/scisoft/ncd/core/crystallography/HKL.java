/*
 * Copyright (c) 2012, 2017 Diamond Light Source Ltd. All rights reserved. This program and the accompanying materials
...
/**
 * This class is a bean to hold d-spacing and, optionally, integer Miller indices (h,k,l). TODO refactor to DSpacing
 * class (but keep as bean) or wrap/delegate to that It is not (currently) a class for using HKLs but rather for storing
 * d-spacings with an optional labelling by HKLs. It does not offer calculation between the values; it just holds them.
 * Often if d is set, h,k and l are not needed but it is up to the user of the data to decide what happens with it. This
 * class has a JScience dependency.
 */
