...
/**
 * CalibrationFactory when we go to e4 like all xxxFactory classes this will become ICalibrationService and be injected.
 * This class has a JScience dependency.
 */
public class CalibrationFactory {
	...
	/**
	 * Reads the calibration standards from disk, creating a new one if required. Only if save is called will they be
	...
	private static File getCalibrantFile() {
		...
		try {
			...
			try {
				...
				logger.error(
						"Cannot create files! Dawn will not function correctly. Please contact you support representative.",
						neOther);
				return null;
			}
		}
	...
	private static CalibrationStandards createCalibrationStandards() {
		...
		if (cs == null || cs.isEmpty()
				|| VersionUtils.isOldVersion(CalibrationStandards.CURRENT_VERSION, cs.getVersion())) {
			cs = new CalibrationStandards();
			cs.setVersion(CalibrationStandards.CURRENT_VERSION); // Versions are so that we can wipe out configurations
																	// with new Dawn versions if we have to
	...
	public static void addCalibrantSelectionListener(CalibrantSelectedListener l) {
		if (listeners == null)
			listeners = new HashSet<CalibrantSelectedListener>(7);
		listeners.add(l);
	}
