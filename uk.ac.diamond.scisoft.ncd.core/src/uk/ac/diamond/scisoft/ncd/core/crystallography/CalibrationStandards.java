...
package uk.ac.diamond.scisoft.ncd.core.crystallography;
...
/**
 * Since we cannot have eclipse preferences in this plugin, but we should access them here, we use the user.home to
 * store the properties and to register them here. This class is a bean which is persisted to disk and can be deep
 * cloned by the UI when the user is creating another calibrant list. This class has a JScience dependency.
 */
public class CalibrationStandards implements Serializable, Cloneable {
	...
	/**
	 * Calling this method saves this CalibrationStandards as the persisted one for all of Dawn. Only call when sure
	 * that this is the required standards.
	 * 
	 * @throws Exception
	 */
	...
	public void setSelectedCalibrant(String selCal, boolean fireListeners) {
		...
		if (fireListeners)
			CalibrationFactory.fireCalibrantSelectionListeners(this, selectedCalibrant);
	}
	...
	/**
	 * Default get default calibrant spacing for named calibrant If not a default calibrant null returned.
	 * 
	 * @param name
	 * @return calibrant
	 */
	 ...
	 * 
	 * @param isModifiable
	 *            if true then adding more calibrants will throw an exception
	 */
