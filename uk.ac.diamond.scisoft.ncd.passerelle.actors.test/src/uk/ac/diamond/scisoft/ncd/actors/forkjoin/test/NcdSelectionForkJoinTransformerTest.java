...
public class NcdSelectionForkJoinTransformerTest {
							...
							for (int l = 0; l < shapeRes[1]; l++) {
							  ...
							  for (int i = 0; i < imageShape[0]; i++) {
								for (int j = 0; j < imageShape[1]; j++) {
									...
									assertEquals(String.format("Test selection frame for (%d, %d, %d)", k, i, j), expected, value, 1e-6*expected);
									assertEquals(String.format("Test selection frame errors for (%d, %d, %d)", k, i, j), expectederrors, errors, 1e-6*expectederrors);
								}
							  }
							}
							}
	...
	@Test
	public void testNcdSelectionForkJoinTransformer() throws FlowAlreadyExecutingException, PasserelleException,
			IllegalActionException, NameDuplicationException {

		NcdMessageSource source = new NcdMessageSource(flow, "MessageSource");
		NcdSelectionForkJoinTransformer selection = new NcdSelectionForkJoinTransformer(flow, "Selection");
		NcdMessageSink sink = new NcdMessageSink(flow, "MessageSink");

		flow.connect(source.output, selection.input);
		flow.connect(selection.output, sink.input);
