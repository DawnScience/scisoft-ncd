...
public class NcdSelectionForkJoinTransformer extends NcdAbstractDataForkJoinTransformer {
	...
	protected long[] getResultDataShape() {
		long[] datDimMake = Arrays.copyOf(frames, frames.length - dimension);
		long[] imageSize = Arrays.copyOfRange(frames, frames.length - dimension, frames.length);
		indexList = NcdDataUtils.createSliceList(format, (int[]) ConvertUtils.convert(datDimMake, int[].class));
		for (int i = 0; i < datDimMake.length; i++) {
			datDimMake[i] = indexList.get(i).length;
		}
		selectedShape = ArrayUtils.addAll(datDimMake, imageSize);
	...
	private class SelectionTask extends RecursiveAction {
		...
		protected void compute() {
			...
			MultidimensionalCounter frameCounter = new MultidimensionalCounter((int[]) ConvertUtils.convert(selectedShape, int[].class));
			...
			while (iter.hasNext()) {
				...
				selectID = H5.H5Sselect_hyperslab(filespaceID, HDF5Constants.H5S_SELECT_SET, writePosition, block, count,
						block);
				if (selectID < 0) {
